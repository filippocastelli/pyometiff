...
class OMETIFFWriter:
    ...
    def write(self):
        self._array, self._dimension_order = self._adjust_dims(self.array, self.dimension_order)
    ...
    def write_stack(self, array, xml_meta):
        ...
        with tifffile.TiffWriter(
            str(self.fpath), bigtiff=self.use_bigtiff) as tif:
            tif.save(array, description=xml_meta, photometric=self.photometric, metadata=None)
    ...
    def gen_meta(self):
        ...
        if channels_dict is not None:
            assert len(channels_dict.keys()) == pixels.SizeC, "Channel label count is differnt than channel count"
    ...
    def _adjust_dims(array, dimension_order):
        ...
        if dimension_order[-2:] != "YX":
            raise InvalidDimensionOrderingError(
                "dimension_order {} expected to be YX. Please transpose your data".format(dimension_order))
        
        # starts in S
        if dimension_order.find("S") > 0:
            raise InvalidDimensionOrderingError(
                "S must be the leading dim in dimension_order {}".format(dimension_order))
            
        # enough dimensions
        if len(dimension_order) < ndims:
            raise InvalidDimensionOrderingError(
                "dimension_order {} must have at least as many dimensions as array shape {}".format(dimension_order, array_shape))
            
        # no letter appears more than once
        if len(set(dimension_order)) != len(dimension_order):
            raise InvalidDimensionOrderingError(
                "A letter appears more than once in dimension_order {}".format(dimension_order))
